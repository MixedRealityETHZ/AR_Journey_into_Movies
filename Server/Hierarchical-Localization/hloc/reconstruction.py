...
def main(
    ...
    sfm_dir.mkdir(parents=True, exist_ok=True) # 创建sfm文件夹，路径为传入的sfm_dir
    database = sfm_dir / "database.db" # 创建db

    logger.info(f"Writing COLMAP logs to {sfm_dir / 'colmap.LOG.*'}")
    pycolmap.logging.set_log_destination(pycolmap.logging.INFO, sfm_dir / "colmap.LOG.") # 创建LOG
